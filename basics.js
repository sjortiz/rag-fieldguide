...
const BASIC_PAGES = [
  ...
  ['Palabras esenciales', 'Un pequeño diccionario para orientarte', 'Selecciona una palabra. Verás qué significa y dónde aparece en el ejemplo.'],
  ['Tu primer RAG', 'Haz de buscador por un momento', 'Elige una pregunta y los documentos que le entregarías al asistente. Observa cómo cambia la respuesta.'],
  ...
  ['Comprueba tu base', 'Ya puedes dar el siguiente paso', 'Responde tres preguntas. Puedes repetirlas y volver a cualquier explicación cuando lo necesites.']
];
...
function basicGo(page){if(page<1||page>6)return;basicPage=page;location.hash=`paso-${page}`;renderBasics();basicEl('#basic-main').focus();window.scrollTo({top:0,behavior:'instant'})}
...
function basicCheckpoint(){
  basicEl('#basic-content').innerHTML=`<form id="basic-quiz">${BASIC_QUIZ.map((q,i)=>`<fieldset class="basic-card"><legend>${i+1}. ${basicEscape(q[0])}</legend>${q[1].map((answer,j)=>`<label class="quiz-option"><input type="radio" name="basic-q-${i}" value="${j}" ${basicQuizAnswers[i]===j?'checked':''}><span>${basicEscape(answer)}</span></label>`).join('')}</fieldset>`).join('')}<button class="primary" type="submit">Comprobar mis respuestas</button></form><div id="basic-quiz-result" aria-live="polite">${basicState.complete?'<div class="feedback good">Ya completaste esta comprobación. Puedes repetirla para repasar.</div>':''}</div><div class="basic-next-path"><h2>Tu ruta a partir de aquí</h2><a href="index.html#lesson-1"><strong>1. Evidencia y respuestas</strong><span>Profundiza en lo que hace confiable una respuesta.</span></a><a href="index.html#lesson-2"><strong>2. Buscar por palabras y significado</strong><span>Continúa con BM25, vectores y la combinación de resultados.</span></a><a href="index.html#lesson-1"><strong>3. Construir en AWS</strong><span>En cada lección, abre “Build on AWS”. Podrás elegir Terraform, CloudFormation o CDK en Python.</span></a><p class="hint">El curso técnico continúa en inglés. Esta introducción y sus ejercicios están en español.</p></div>`;
  ...
  basicEl('#basic-quiz').onsubmit=e=>{e.preventDefault();if(BASIC_QUIZ.some((_,i)=>!Object.hasOwn(basicQuizAnswers,i))){basicEl('#basic-quiz-result').innerHTML='<div class="feedback warn">Selecciona una respuesta para cada pregunta antes de comprobarlas.</div>';return}const score=BASIC_QUIZ.filter((q,i)=>basicQuizAnswers[i]===q[2]).length;if(score===3){basicState.complete=true;basicSave();basicEl('#basic-progress-label').textContent='Base completada ✓'}basicEl('#basic-quiz-result').innerHTML=`<div class="feedback ${score===3?'good':'warn'}"><strong>${score} de 3 respuestas correctas.</strong><p>${score===3?'Ya tienes una base para entrar al curso técnico.':'Revisa las explicaciones y vuelve a intentarlo.'}</p>${BASIC_QUIZ.map((q,i)=>`<p><strong>${i+1}. ${basicQuizAnswers[i]===q[2]?'Correcta':'Para repasar'}:</strong> ${basicEscape(q[3])}</p>`).join('')}${score===3?'<a href="index.html#lesson-1">Continuar a la lección 1 →</a>':''}</div>`};
}
basicEl('#basic-prev').onclick=()=>basicGo(basicPage-1);
basicEl('#basic-next').onclick=()=>{if(basicPage<6)basicGo(basicPage+1);else location.href='index.html#lesson-1'};
window.addEventListener('hashchange',()=>{const p=basicHashPage()||1;if(p!==basicPage){basicPage=p;renderBasics();basicEl('#basic-main').focus();window.scrollTo({top:0,behavior:'instant'})}});
renderBasics();
